// This file is all the different choices you can make in each room. It changes
// depending on which rooms you've already visited.
// Let's say you're in the entrance, and haven't visited any other room. Then
// it'll show you all options as visiting a door. But if you've visited the
// Kitchen, one of the options will be "go to the Kitchen" instead of
// "go through the door on the left"

void hiddenRoomNotFoundBasement() {
  print('1: Examine the room');
  print('2: Go back to the hall');
  print('3: Look at inventory');
}

void hiddenRoomFoundBasement() {
  print('1: Examine the room');
  print('2: Go into the hidden room');
  print('3: Go back to the hall');
  print('4: Look at inventory');
}

void visitingBedroomBobAlive() {
  print('1: Look at Bob');
  print('2: Examine the room');
  print('3: Go to the Living Room');
  print('4: Look at inventory');
}

void visitingBedroomBobDead() {
  print('1: Look at Bobs body');
  print('2: Examine the room');
  print('3: Go to the Living Room');
  print('4: Look at inventory');
}

void noRoomsVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go through the door on the left');
  print('3: Go through the door straight ahead');
  print('4: Go through the door on the right');
  print('5: Look at inventory');
}

void kitchenVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go to the kitchen');
  print('3: Go through the door straight ahead');
  print('4: Go through the door on the right');
  print('5: Look at inventory');
}

void mainHallVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go through the door on the left');
  print('3: Go to the Main Hall');
  print('4: Go through the door on the right');
  print('5: Look at inventory');
}

void libraryVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go through the door on the left');
  print('3: Go through the door straight ahead');
  print('4: Go to the Library');
  print('5: Look at inventory');
}

void kitchenAndMainHallVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go to the Kitchen');
  print('3: Go to the Main Hall');
  print('4: Go through the door on the right');
  print('5: Look at inventory');
}

void kitchenAndLibraryVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go to the Kitchen');
  print('3: Go through the door straight ahead');
  print('4: Go to the Library');
  print('5: Look at inventory');
}

void mainHallAndLibraryVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go through the door on the left');
  print('3: Go to the Main Hall');
  print('4: Go to the Library');
  print('5: Look at inventory');
}

void allRoomsVisitedEntrance() {
  print('1: Examine the room');
  print('2: Go to the Kitchen');
  print('3: Go to the Main Hall');
  print('4: Go to the Library');
  print('5: Look at inventory');
}

void noNewRoomsGarden() {
  print('1: Examine the Garden and Dog');
  print('2: Go to the small house in the corner');
  print('3: Go to the Hall');
  print('4: Look at inventory');
}

void kennelVisitedGarden() {
  print('1: Examine the Garden and Dog');
  print('2: Go to the Kennel');
  print('3: Go to the Hall');
  print('4: Look at inventory');
}

void noNewRoomsHall() {
  print('1: Examine the room');
  print('2: Go through the door leading outside');
  print('3: Go down the stairs');
  print('4: Go to the Main Hall');
  print('5: Look at inventory');
}

void gardenVisitedHall() {
  print('1: Examine the room');
  print('2: Go to the Garden');
  print('3: Go down the stairs');
  print('4: Go to the Main Hall');
  print('5: Look at inventory');
}

void basementVisitedHall() {
  print('1: Examine the room');
  print('2: Go through the door leading outside');
  print('3: Go to the Basement');
  print('4: Go to the Main Hall');
  print('5: Look at inventory');
}

void allRoomsVisitedHall() {
  print('1: Examine the room');
  print('2: Go to the Garden');
  print('3: Go to the Basement');
  print('4: Go to the Main Hall');
  print('5: Look at inventory');
}

void visitingHiddenRoom() {
  print('1: Examine the room');
  print('2: Go back to the Basement');
  print('3: Look at inventory');
}

void visitingKennel() {
  print('1: Examine the room');
  print('2: Go to the Garden');
  print('3: Look at inventory');
}

void noNewRoomsKitchen() {
  print('1: Examine the room');
  print('2: Go through the door in the corner');
  print('3: Go to the Entrance');
  print('4: Look at inventory');
}

void storageVisitedKitchen() {
  print('1: Examine the room');
  print('2: Go to the Storage');
  print('3: Go to the Entrance');
  print('4: Look at inventory');
}

void visitingLibrary() {
  print('1: Examine the room');
  print('2: Go to the Entrance');
  print('3: Look at inventory');
}

void noNewRoomsLivingRoom() {
  print('1: Examine the room');
  print('2: Go through the door to the left');
  print('3: Go through the door straight ahead');
  print('4: Go back to the Main Hall');
  print('5: Look at inventory');
}

void bedroomVisitedLivingRoom() {
  print('1: Examine the room');
  print('2: Go to the Bedroom');
  print('3: Go through the door straight ahead');
  print('4: Go back to the Main Hall');
  print('5: Look at inventory');
}

void cakeRoomVisitedLivingRoom() {
  print('1: Examine the room');
  print('2: Go through the door to the left');
  print('3: Go to the Giant Safe');
  print('4: Go back to the Main Hall');
  print('5: Look at inventory');
}

void allRoomsVisitedLivingRoom() {
  print('1: Examine the room');
  print('2: Go to the Bedroom');
  print('3: Go to the Giant Safe');
  print('4: Go back to the Main Hall');
  print('5: Look at inventory');
}

void noNewRoomsMainHall() {
  print('1: Examine the room');
  print('2: Go through the door straight ahead');
  print('3: Go through the door on the right');
  print('4: Go back to the entrance');
  print('5: Look at inventory');
}

void hallVisitedMainHall() {
  print('1: Examine the room');
  print('2: Go to the Hall');
  print('3: Go through the door on the right');
  print('4: Go back to the entrance');
  print('5: Look at inventory');
}

void livingRoomVisitedMainHall() {
  print('1: Examine the room');
  print('2: Go through the door straight ahead');
  print('3: Go to the Living Room');
  print('4: Go back to the entrance');
  print('5: Look at inventory');
}

void allRoomsVisitedMainHall() {
  print('1: Examine the room');
  print('2: Go to the Hall');
  print('3: Go to the Living Room');
  print('4: Go back to the entrance');
  print('5: Look at inventory');
}

void visitingStorage() {
  print('1: Examine the room');
  print('2: Go to the Kitchen');
  print('3: Look at inventory');
}
